"""Coordinator for computed adaptive lighting values."""
...
class SimpleAdaptiveCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    """Maintain and refresh computed adaptative values."""
    ...
    def enabled(self) -> bool:
        """Return current runtime enabled flag."""
        return self._enabled

    async def async_set_enabled(self, enabled: bool) -> None:
        """Toggle the runtime flag and refresh entities."""
    ...
    async def async_apply_now(self) -> None:
        """Apply current profile immediately and refresh entities."""
        snapshot = self._build_snapshot()
        if self._enabled:
            snapshot["last_apply"] = await self._async_apply_to_lights(snapshot)
        self.async_set_updated_data(snapshot)
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        """Compute adaptive values and apply them when enabled."""
        snapshot = self._build_snapshot()
        if self._enabled:
            snapshot["last_apply"] = await self._async_apply_to_lights(snapshot)
        return snapshot
    ...
    def _get_value(self, key: str, default: Any) -> Any:
        """Read options first, then fallback to entry data."""
        return self.entry.options.get(key, self.entry.data.get(key, default))

    def _build_snapshot(self) -> dict[str, Any]:
        """Build state payload exposed by sensor/switch."""
        ...
        target_lights = list(self._get_value(CONF_TARGET_LIGHTS, []))

        sun_state = self.hass.states.get("sun.sun")
        sun_elevation = float(sun_state.attributes.get("elevation", 0.0)) if sun_state else 0.0

        # Map sun elevation from [-6, 45] into [0, 1].
    ...
    async def _async_apply_to_lights(self, snapshot: dict[str, Any]) -> str:
        """Apply profile to currently-on target lights."""
